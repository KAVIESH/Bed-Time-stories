...
export default class ReadStoryScreen extends React.Component{
  ...
  componentDidMount = async () => {
    const allStories = await db.collection("bookDetails").limit(10).get();
  ...
  fetchMoreStories = async (text) => {
    const story = await db.collection("bookDetails").where('title', '==', text).startAfter(this.state.lastVisibleStory).limit(10).get();
    story.docs.map((doc) => {
      this.setState({
        book: [...this.state.book, doc.data()],
        lastVisibleStory: doc
  ...
  searchFilterFunction = async (text) => {
    const story = await db.collection("bookDetails").where('title', '==', text).get();
    story.docs.map((doc) => {
      this.setState({
        book: [...this.state.book, doc.data()],
        lastVisibleStory: doc
